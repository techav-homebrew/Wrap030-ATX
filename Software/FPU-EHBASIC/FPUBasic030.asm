...
; This is just to make it easier to actually burn this to ROM.
...
; (techav) I'm adding these here to make it easier for an external loader 
; to run this program after loading it into memory.
; The external loader will need to do the following:
;		load BASIC into memory
;		copy base memory segment address to A0
;		copy size of memory allocated to BASIC to D0
;		fetch the LAB_COLD address from this location
;		jump to the fetched address
...
; ACIA was already initialized by the monitor, so skip re-initializing
...
LAB_sizok:
	...
	; 	flagFPU(a3) = 0	: FPU exists & working
	;	flagFPU(a3) = 1	: FPU does not exist
	MOVEM.l	a0-a1,-(sp)	; save working registers
	MOVE.l	#$02C,a0	; get the F-trap vector address
	MOVE.l	(a0),-(sp)	; save the current F-trap vector address
	LEA		vecFPUNO(pc),a1	; calculate the new F-trap vector
	MOVE.l	a1,(a0)		; save the new F-trap vector
	CLR.b	flagFPU(a3)	; clear FPU flag
	...
	BRA  	bootContinue
vecFPUNO:
	MOVE.b	#1,flagFPU(a3)	; set no FPU flag
	ADD.l	#4,2(sp)	; increment return pointer
...
LAB_1B7D:
	...
	BEQ 	LAB_1B84	; branch if was $64 (< function can be string)
...
LAB_VAL:
	BSR	LAB_22B6	; pop string off descriptor stack or from memory
			; returns with d0 = length, a0 = pointer
	BEQ 	LAB_VALZ	; string was null so set result = $00
...
LAB_SUBTRACT:
	...
	BEQ 	.LAB_SUB_NOFPU	; if no FPU then emulate
	FAC2toD0				; get FAC2
...
.LAB_SUB_NOFPU:
	...
	BRA  	LAB_ADD_NOFPU	; fall through to no-FPU addition
...
LAB_ADD:
	FPUTEST					; check for FPU
	BEQ.S	LAB_ADD_NOFPU	; if no FPU then emulate
	FAC2toD0				; get FAC2
	FMOVE.s	D0,FP0			; and copy to FP0
	FAC1toD0				; get FAC1
	FADD.s	D0,FP0			; add FAC1 to FAC2
	FMOVE.s	FP0,D0			; copy result to D0
	D0toFAC1				; and save resulin FAC1
	RTS
...
LAB_LOG:
	FPUTEST
	BEQ 	.LAB_LOG_NOFPU		; jump to no FPU
...
LAB_MULTIPLY:
	FPUTEST					; check for FPU
	BEQ 	.LAB_MULT_NOFPU	; if no FPU then emulate
	FAC2toD0				; get FAC2
	FMOVE.s	D0,FP0			; and copy to FP0
	FAC1toD0				; get FAC1
...
LAB_DIVIDE:
	...
	BEQ 	.LAB_DIV_NOFPU	; emulate if no FPU
...
LAB_2831:
	...
	MOVE.l	d1,-(sp)	; save d1
	MOVEQ.l	#$A0,d1	; set for no floating bits
...
LAB_INT:
	MOVEQ.l	#$A0,d0	; set for no floating bits
	SUB.b	FAC1_e(a3),d0	; subtract FAC1 exponent
...
LAB_TAN:
	FPUTEST					; check FPU presence
	BEQ 	.LAB_TAN_NOFPU	; emulate if no FPU
	FAC1toD0				; get FAC1 in D0
...
LAB_COS:
	FPUTEST					; check FPU presence
	BEQ 	.LAB_COS_NOFPU	; if no FPU then emulate
	FAC1toD0				; get FAC1 in D0
...
LAB_SIN:
	FPUTEST					; check FPU presence
	BEQ 	.LAB_SIN_NOFPU	; if no FPU then emulate
	FAC1toD0				; get FAC1 in D0
...
LAB_ATN:
	...
	BEQ 	.LAB_ATN_NOFPU	; and emulate if no FPU
	FAC1toD0				; get FAC1
...
LAB_ATLE:
	MOVE.l	FAC1_m(a3),d0	; get FAC1 mantissa
	MOVEQ.l	#$82,d1	; set to correct exponent
...
LAB_SQR:
	FPUTEST					; check FPU presence
	BEQ 	.LAB_SQR_NOFPU	; and emulate if none
	FAC1toD0				; get FAC1
